...
public class MultiMCLauncher
{
	 ...
	 *            Username, Session ID.
	 * 	      The first argument should be either 'true' for a server instance or a username.
	 */
	// TODO add server options, etc
	public static void main(String[] args)
	{	
		
		if (args.length < 3 && !Boolean.parseBoolean(args[0]))
		{
			System.out.println("Not enough arguments.");
			System.exit(-1);
		}
		boolean isServer = Boolean.parseBoolean(args[0]);
		// just in case someone is called 'true'
		if (args.length >= 3) {
			System.out.println("Detected user with name 'true', not starting a server...");
			isServer = false;
		}
		String userName = "";
		String sessionId = "";
	...
	//					e.printStackTrace();
			...
			try
			{
				System.out.println("Loading jars...");
					...
					for (int i = 0; i < urls.length; i++)
					{
						try
						{
							File f = new File(new File(cwd), jarFiles[i]);
							urls[i] = f.toURI().toURL();
							System.out.println("Loading URL: " + urls[i].toString());
						} catch (MalformedURLException e)
						{
		//					e.printStackTrace();
					...
					try
					{
						mc = cl.loadClass("net.minecraft.server.MinecraftServer");
						
						Field f = getMCPathField(mc);
						
						if (f == null)
						{
							System.err.println("Could not find Minecraft Server path field. Launch failed.");
							System.exit(-1);
						}
						...
						f.set(null, new File(cwd));
						// And set it.
						System.out.println("Fixed Minecraft Server Path: Field was " + f.toString());
					}
					catch (ClassNotFoundException e)
					{
						System.err.println("Can't find main class. Searching...");
